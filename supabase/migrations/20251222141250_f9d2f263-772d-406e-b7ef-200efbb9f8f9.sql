...
DECLARE
  ...
  snapshot jsonb;
BEGIN
  -- Ensure day_reports row exists for report_date
  ...
  FOR si IN SELECT item_id, name, current_stock FROM public.stock_items LOOP
    ...
    -- set or overwrite opening for the item
    snapshot := jsonb_set(
      snapshot,
      ARRAY[si.name],
      COALESCE(
    ...
    -- ensure sold key exists with default 0 if missing
    ...
    -- recompute closing = GREATEST(opening - sold, 0)
    snapshot := (
